...
import { FormDepositActions, defaultI18n, FormDepositActionsProps } from '.'
...
export const Default: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Should contain label of total depositing amount', async ({ args }) => {
      const canvas = within(canvasElement)
      const text = canvas.getByText(`${defaultI18n.totalDeposit} ${args.depositingAmount}`)
...
export const WithoutDepositAmount: Story = {
  ...
  play: async ({ canvasElement, args }) => {
    const canvas = within(canvasElement)
    const text = canvas.queryByText(`${defaultI18n.totalDeposit} ${args.depositingAmount}`)
